...
*
*******************************************************************************/
void mpi_gaspi_parallel_init(int argc, char *argv[], int *rank, int *ndom)
{
  int provided, required;
  gaspi_rank_t gaspi_rank, gaspi_ndom;
  ...
  GASPICHECK(gaspi_proc_init(GASPI_BLOCK));
  GASPICHECK(gaspi_proc_rank(&gaspi_rank));
  GASPICHECK(gaspi_proc_num(&gaspi_ndom));

  CHECK(*rank == gaspi_rank);
  CHECK(*ndom == gaspi_ndom);
  ...
  if(DBG_PRINT)
  {
    ...
    GASPICHECK(gaspi_queue_num(&queue_num));
    GASPICHECK(gaspi_queue_max(&queue_max));
    GASPICHECK(gaspi_queue_size_max(&queue_size_max));
    GASPICHECK(gaspi_notification_num(&notif_num));

    printf("[%d] gaspi_queue_num: %d\n", *rank, queue_num);
    printf("[%d] gaspi_queue_max: %d\n", *rank, queue_max);
    printf("[%d] gaspi_queue_size_max: %d\n", *rank, queue_size_max);
    printf("[%d] gaspi_notification_num: %d\n", *rank, notif_num);
  }
} /* mpi_gaspi_parallel_init() */

/*******************************************************************************
*
*******************************************************************************/
void mpi_gaspi_parallel_end(void)
{
  GASPICHECK(gaspi_barrier(GASPI_GROUP_ALL, GASPI_BLOCK));
  GASPICHECK(gaspi_proc_term(GASPI_BLOCK));

  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Finalize();
} /* mpi_gaspi_parallel_end() */
